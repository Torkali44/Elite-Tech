...
<div class="flex bg-mist rounded-lg p-1 mb-6 text-sm font-bold">
    <a href="{{ route('login') }}" class="flex-1 text-center py-2.5 rounded-md bg-white text-primary">تسجيل الدخول</a>
    <a href="{{ route('register') }}" class="flex-1 text-center py-2.5 rounded-md text-tertiary hover:text-primary">حساب جديد</a>
</div>
...
<form action="{{ route('login') }}" method="POST" class="space-y-4">
    ...
    <div>
        ...
        <input type="email" name="email" value="{{ old('email') }}" placeholder="name@example.com" class="input" required autofocus>
    </div>
    <div>
        <div class="flex items-center justify-between mb-1.5">
            <label class="block text-sm font-bold text-primary">كلمة المرور</label>
            <a href="{{ route('password.request') }}" class="text-xs text-secondary font-semibold hover:underline">نسيت كلمة المرور؟</a>
        </div>
