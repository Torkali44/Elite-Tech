...
@section('title', $track['title'] . ' — التفاصيل')
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
        <nav class="text-xs text-slate-500 font-bold mb-2 flex items-center gap-1.5">
            <a href="{{ route('career-tracks.index') }}" class="hover:text-primary transition-colors">المسارات المهنية</a>
            <span>‹</span>
            <span class="text-primary">{{ $track['title'] }}</span>
        </nav>
        <h1 class="text-2xl sm:text-4xl font-black text-primary flex items-center gap-3">
            <span>{{ $track['title'] }} - التفاصيل</span>
        </h1>
        <p class="text-sm text-slate-500 font-medium mt-1">{{ $track['subtitle'] }}</p>
    </div>

    <!-- Status Badge -->
    <div class="shrink-0">
        <span class="px-4 py-2 rounded-xl text-sm font-extrabold shadow-sm {{ $track['statusColor'] }}">
            {{ $track['statusLabel'] }}
        </span>
    </div>
</div>

@if($track['needsAction'])
...
<div class="grid lg:grid-cols-12 gap-8 items-start">
    ...
    <div class="lg:col-span-8 space-y-6">
            ...
            <div class="relative space-y-8 pr-2">
                ...
                <div class="relative flex items-start gap-5">
                    ...
                    <div class="relative z-10 w-11 h-11 rounded-full grid place-items-center text-sm font-extrabold shrink-0 shadow-sm transition-transform hover:scale-110
                        @if($step['state'] === 'done') bg-blue-600 text-white ring-4 ring-blue-100
                        @elseif($step['state'] === 'current') bg-secondary text-white ring-4 ring-orange-100 animate-pulse
                    ...
                    <div class="flex-1 bg-slate-50/70 border border-slate-100 rounded-2xl p-5 hover:bg-white hover:shadow-md transition-all duration-200">
                        <div class="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                            <h4 class="font-extrabold text-primary text-base sm:text-lg">{{ $step['title'] }}</h4>
                            <span class="badge text-xs font-bold
                                @if($step['state'] === 'done') bg-blue-50 text-blue-700 border border-blue-200
                                @elseif($step['state'] === 'current') bg-orange-50 text-secondary border border-orange-200
                                @else bg-slate-100 text-slate-500 border border-slate-200 @endif">
                                {{ $step['stateLabel'] }}
                            </span>
                        </div>
                        <p class="text-sm text-slate-600 font-medium leading-relaxed">{{ $step['desc'] }}</p>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="card p-6 shadow-card">
            ...
            <form action="{{ route('career-tracks.update', $track['slug']) }}" method="POST" class="space-y-4">
                @csrf
                <div>
    ...
    <div class="lg:col-span-4 space-y-6">
        ...
        <div class="card p-6 shadow-card space-y-4">
            ...
            <div class="space-y-3.5 text-sm">
                <div>
                    ...
                    <div class="font-extrabold text-primary text-base">{{ $track['projectName'] ?? 'منصة إبداع للتقنيات العقارية' }}</div>
                </div>
                ...
                <div>
                    ...
                    <span class="badge bg-slate-100 text-slate-700 font-bold border border-slate-200">
                        {{ $track['projectCategory'] ?? 'PropTech' }}
                    </span>
                </div>

                <div>
                    <div class="text-xs text-slate-400 font-bold mb-1">تاريخ التقديم</div>
                    <div class="font-semibold text-slate-700">{{ $track['submissionDate'] ?? '12 أكتوبر 2023' }}</div>
                </div>
            </div>

            <div class="pt-2">
                <a href="{{ route('ideas.index') }}" class="btn-outline w-full text-center text-sm !py-2.5">
                    👁️ عرض الطلب كاملاً
                </a>
            </div>
        </div>
        ...
        <div class="card p-6 shadow-card">
            ...
            <a href="{{ route('network.index') }}" class="text-secondary font-extrabold text-sm hover:underline inline-flex items-center gap-1">
