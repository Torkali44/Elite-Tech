...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="grid lg:grid-cols-[260px_1fr] gap-6">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            ...
            <a href="{{ route('community.show',$m['id']) }}" class="card p-5 hover:shadow-card-hover transition">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-12 h-12 rounded-full bg-primary/10 grid place-items-center text-primary font-bold">{{ mb_substr($m['name'],0,1) }}</div>
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-primary truncate">{{ $m['name'] }}</div>
                        <div class="text-xs text-tertiary">{{ $m['title'] }}</div>
                    </div>
                    <span class="badge bg-emerald-50 text-emerald-600 text-[10px]">Elite</span>
                </div>
                <h4 class="font-bold text-primary text-sm mb-2">{{ $m['project'] }}</h4>
                <p class="text-xs text-tertiary line-clamp-2 mb-3">{{ $m['bio'] }}</p>
    ...
    <div class="flex items-center justify-center gap-2 mt-10 text-sm">
        ...
        <a href="?page={{$p}}" class="w-9 h-9 grid place-items-center rounded-lg {{ $p==1 ? 'bg-primary text-white' : 'bg-white border border-slate-200 text-tertiary hover:border-primary' }}">{{ $p }}</a>
        @endforeach
    </div>
</div>
@endsection
