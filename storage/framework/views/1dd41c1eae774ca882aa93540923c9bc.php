<?php $__env->startSection('title', __('ideas.bank_title') . ' — Elite Tech Community'); ?>
<?php $__env->startSection('description', __('ideas.bank_subtitle')); ?>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
            <h1 class="text-3xl font-extrabold text-primary mb-2"><?php echo e(__('ideas.bank_title')); ?></h1>
            <p class="text-sm text-tertiary max-w-2xl leading-relaxed"><?php echo e(__('ideas.bank_subtitle')); ?></p>
        </div>
        <?php if(auth()->guard()->check()): ?>
            <a href="<?php echo e(route('ideas.create')); ?>" class="btn-secondary text-sm !py-2.5 !px-5 shrink-0"><?php echo e(__('ideas.create_new')); ?></a>
        <?php else: ?>
            <button type="button" class="btn-secondary text-sm !py-2.5 !px-5 shrink-0"
                    @click="gateOpen=true; gateMsg='<?php echo e(app()->getLocale()==='ar' ? 'لنشر فكرة تحتاج حساباً، ومسار صاحب الفكرة يتطلب KYC قبل النشر.' : 'You need an account to publish an idea. Idea Owner path requires KYC before publishing.'); ?>'">
                <?php echo e(__('ideas.create_new')); ?>
        ...
        <?php $__currentLoopData = [
            ...
            <?php if(in_array($key, ['my', 'favorites'], true) && !auth()->check()): ?>
                <button type="button"
                        ...
                        @click="gateOpen=true; gateMsg='<?php echo e(app()->getLocale()==='ar' ? 'سجّل الدخول لعرض ' . $label : 'Sign in to view ' . $label); ?>'">
                    <?php echo e($label); ?>
    ...
    <div class="grid sm:grid-cols-3 gap-4 mb-8">
        <div class="card p-4 flex items-center justify-between">
            <div>
                <div class="text-xs text-tertiary mb-0.5"><?php echo e(app()->getLocale()==='ar' ? 'إجمالي أفكار المجتمع' : 'Total Community Ideas'); ?></div>
        ...
        <div class="card p-4 flex items-center justify-between">
            <div>
                <div class="text-xs text-tertiary mb-0.5"><?php echo e(app()->getLocale()==='ar' ? 'المساهمون النشطون' : 'Active Contributors'); ?></div>
        ...
        <div class="card p-4 flex items-center justify-between">
            <div>
                <div class="text-xs text-tertiary mb-0.5"><?php echo e(app()->getLocale()==='ar' ? 'الأكثر بحثاً' : 'Most Searched'); ?></div>
    ...
    <div class="grid lg:grid-cols-[260px_1fr] gap-6 items-start">
        ...
        <aside class="card p-5 sticky top-20">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h3 class="font-extrabold text-primary text-sm"><?php echo e(app()->getLocale()==='ar' ? 'تصفية النتائج' : 'Filter Results'); ?></h3>
            </div>
            ...
            <form method="GET" action="<?php echo e(route('ideas.index')); ?>" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-tertiary mb-1.5"><?php echo e(__('general.search')); ?></label>
                    <input type="text" name="q" value="<?php echo e(request('q')); ?>" class="input !py-2 text-sm" placeholder="<?php echo e(__('ideas.search_placeholder')); ?>">
                </div>

                <div>
                    <label class="block text-xs font-bold text-tertiary mb-1.5"><?php echo e(__('general.category')); ?></label>
                    <select name="category" class="input !py-2 text-sm">
                        <option value=""><?php echo e(__('ideas.all_categories')); ?></option>
                ...
                <div>
                    <div class="text-xs font-bold text-tertiary mb-2"><?php echo e(app()->getLocale()==='ar' ? 'ترتيب حسب' : 'Sort by'); ?></div>
                    <label class="flex items-center gap-2 text-sm text-primary mb-2 cursor-pointer">
                        ...
                        <?php echo e(app()->getLocale()==='ar' ? 'الأحدث' : 'Newest'); ?>
                    ...
                    <label class="flex items-center gap-2 text-sm text-primary cursor-pointer">
                        ...
                        <?php echo e(app()->getLocale()==='ar' ? 'الأكثر شهرة' : 'Most Popular'); ?>

                    </label>
                </div>

                <button type="submit" class="btn-primary w-full text-sm !py-2.5"><?php echo e(__('ideas.filter_button')); ?></button>
                <a href="<?php echo e(route('ideas.index', $tab && $tab !== 'all' ? ['tab' => $tab] : [])); ?>"
                   class="btn-ghost w-full text-sm text-center block !py-2"><?php echo e(app()->getLocale()==='ar' ? 'إعادة تعيين' : 'Reset'); ?></a>
            </form>
        </aside>
        ...
        <div>
            <?php if($ideas->isEmpty()): ?>
                <div class="card p-12 text-center">
                    <h3 class="font-extrabold text-primary text-lg mb-2"><?php echo e(__('ideas.no_ideas')); ?></h3>
                    <p class="text-sm text-tertiary mb-4"><?php echo e(app()->getLocale()==='ar' ? 'جرّب تغيير الفلاتر أو التبويب.' : 'Try changing filters or tab.'); ?></p>
                    <a href="<?php echo e(route('ideas.index')); ?>" class="btn-outline text-sm"><?php echo e(app()->getLocale()==='ar' ? 'إعادة ضبط' : 'Reset'); ?></a>
                </div>
            <?php else: ?>
