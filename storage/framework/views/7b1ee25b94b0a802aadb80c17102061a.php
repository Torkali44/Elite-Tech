<?php $__env->startSection('title','طلبات KYC'); ?>
...
<div class="mb-6 flex flex-wrap items-end justify-between gap-3">
    ...
    <form class="flex gap-2">
        <select name="status" class="input !py-2" onchange="this.form.submit()">
            <option value="pending" <?php if(request('status','pending')==='pending'): echo 'selected'; endif; ?>>معلّق</option>
            ...
            <option value="" <?php if(request()->has('status') && request('status')===''): echo 'selected'; endif; ?>>الكل</option>
        </select>
    </form>
</div>
...
<div class="card p-5" x-data="{ reject:false, notes:false }">
    <div class="flex flex-wrap gap-4 items-start justify-between">
        <div>
            ...
            <?php if($v->user?->admin_notes): ?>
                <p class="text-xs text-tertiary mt-2 bg-mist rounded-lg px-2 py-1">ملاحظة داخلية: <?php echo e($v->user->admin_notes); ?></p>
            <?php endif; ?>
        </div>
    ...
    <div class="grid sm:grid-cols-3 gap-3 mt-4">
        ...
        <?php else: ?>
            <?php $__currentLoopData = [['أمامية','id_front',$v->id_front],['خلفية','id_back',$v->id_back],['Selfie','selfie',$v->selfie]]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as [$label,$fieldKey,$path]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="bg-mist rounded-xl p-3 text-center text-xs">
                    <div class="font-bold text-primary mb-1"><?php echo e($label); ?></div>
                    <?php if($path): ?>
                        <a href="<?php echo e(route('admin.verifications.file', [$v->id, $fieldKey])); ?>" target="_blank" class="text-secondary font-bold underline">عرض المستند</a>
                    <?php else: ?>
    ...
    <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-mist">
        ...
        <form method="POST" action="<?php echo e(route('admin.users.notes', $v->user_id)); ?>" class="flex-1 flex gap-2 min-w-[200px]"><?php echo csrf_field(); ?>
            <input name="admin_notes" class="input !py-2 text-xs" placeholder="ملاحظة داخلية..." value="<?php echo e($v->user->admin_notes); ?>">
