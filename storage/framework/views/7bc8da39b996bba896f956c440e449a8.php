...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'لوحة التحكم'); ?> — Elite Tech Community</title>
...
<?php endif; ?>
                    <div class="min-w-0 flex-1">
                        <div class="font-extrabold text-primary text-sm truncate"><?php echo e($user->name ?? 'عضو'); ?></div>
                        <div class="text-xs text-tertiary truncate"><?php echo e($roleLabels[$primaryRole] ?? ($user->title ?? 'عضو')); ?></div>
                    </div>
                </div>
                ...
                <?php if(!$user->hasRole('idea_seeker') || $user->hasRole('idea_owner')): ?>
                    ...
                    <?php else: ?>
                        <a href="<?php echo e(route('verification.kyc')); ?>" class="mt-3 badge bg-secondary/15 text-secondary w-full justify-center py-1.5 hover:bg-secondary/25 transition">
                            أكمل التوثيق الآن
                        </a>
                    <?php endif; ?>
                <?php endif; ?>
        ...
        <header class="no-print bg-white border-b border-mist sticky top-0 z-30">
            <div class="flex items-center justify-between px-4 lg:px-8 h-14 gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="text-sm font-extrabold text-primary"><?php echo e($roleLabels[$primaryRole] ?? 'لوحة العضو'); ?></div>
                        <div class="text-xs text-tertiary">مرحباً، <?php echo e($user->name); ?></div>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <?php if($user->hasRole('idea_owner')): ?>
                        <a href="<?php echo e(route('ideas.create')); ?>" class="btn-secondary text-sm !py-2 !px-4">+ فكرة جديدة</a>
                    <?php elseif($user->hasRole('idea_seeker') && ! $user->isKycApproved()): ?>
                        <a href="<?php echo e(route('verification.kyc', ['purpose' => 'implement'])); ?>" class="btn-secondary text-sm !py-2 !px-4">أكمل KYC</a>
                    <?php else: ?>
                        <a href="<?php echo e(route('auth.path')); ?>" class="btn-outline text-sm !py-2 !px-4 hidden sm:inline-flex">تغيير المسار</a>
                    <?php endif; ?>
                </div>
            </div>
        </header>
