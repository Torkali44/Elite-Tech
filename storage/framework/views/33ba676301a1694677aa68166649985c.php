...
<div class="flex bg-mist rounded-lg p-1 mb-6 text-sm font-bold">
    <a href="<?php echo e(route('login')); ?>" class="flex-1 text-center py-2.5 rounded-md text-tertiary hover:text-primary">تسجيل الدخول</a>
    <a href="<?php echo e(route('register')); ?>" class="flex-1 text-center py-2.5 rounded-md bg-white text-primary">حساب جديد</a>
</div>
...
<form action="<?php echo e(route('register')); ?>" method="POST" class="space-y-4">
    ...
    <div>
        ...
        <input type="email" name="email" value="<?php echo e(old('email')); ?>" placeholder="name@example.com" class="input" required>
    </div>
    ...
    <label class="flex items-start gap-2.5 text-sm text-tertiary">
        ...
        <span>أوافق على <a href="<?php echo e(route('terms')); ?>" class="text-secondary underline" target="_blank">الشروط والأحكام</a>
        و <a href="<?php echo e(route('privacy')); ?>" class="text-secondary underline" target="_blank">سياسة الخصوصية</a></span>
    </label>
    <button type="submit" class="btn-primary w-full">إنشاء الحساب</button>
</form>

<p class="text-center text-sm text-tertiary mt-6">
    لديك حساب؟ <a href="<?php echo e(route('login')); ?>" class="text-primary font-bold hover:underline">تسجيل الدخول</a>
</p>
